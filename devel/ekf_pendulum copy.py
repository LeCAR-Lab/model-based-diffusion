...
class PendulumParams(NamedTuple):
    ...
    dynamics_covariance: Float[Array, "state_dim state_dim"] = jnp.zeros((2, 2))
    emission_function: Callable = lambda x, u: jnp.concat([x, u], axis=0)
    emission_covariance: Float[Array, "emission_dim"] = jnp.eye(3) * (r**2)
