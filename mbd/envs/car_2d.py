...
def car_dynamics(x, u):
    return jnp.array(
        [
            x[3] * jnp.sin(x[2]) * 1.0,  # x_dot
            x[3] * jnp.cos(x[2]) * 1.0,  # y_dot
            x[3] * u[0] * jnp.pi / 3 * 2.0,  # theta_dot
            u[1] * 6.0,  # v_dot
        ]
    )
...
class Car2d:
    def __init__(self):
        ...
        self.x0 = jnp.array([-0.5, 0.0, jnp.pi/2, 0.0])
    ...
    def eval_xref_logpd(self, xs):
        ...
        logpd = -(jnp.linalg.norm(xs_err, axis=-1)**2).mean(axis=-1)
        return logpd
